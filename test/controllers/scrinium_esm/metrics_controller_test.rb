...
module ScriniumEsm
  class MetricsControllerTest < ActionController::TestCase
    setup do
      @metric = scrinium_esm_metrics(:one)
    ...
    test "should create metric" do
      assert_difference('Metric.count') do
        post :create, metric: { description: @metric.description, name: @metric.name }
      end
    ...
    test "should update metric" do
      patch :update, id: @metric, metric: { description: @metric.description, name: @metric.name }
      assert_redirected_to metric_path(assigns(:metric))
    end
